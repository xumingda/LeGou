...
package com.xq.LegouShop.support;
...
public class PercentLayoutHelper
{
    ...
    public void adjustChildren(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        {
            if(DEBUG) Log.d(TAG, "adjustChildren: " + mHost + " widthMeasureSpec: "
        ...
        if(DEBUG) Log.d(TAG, "widthHint = " + widthHint + " , heightHint = " + heightHint);
        for (int i = 0, N = mHost.getChildCount(); i < N; i++)
        {
            ...
            if (Log.isLoggable(TAG, Log.DEBUG))
            {
                if(DEBUG) Log.d(TAG, "should adjust " + view + " " + params);
            }
    ...
    {
        if(DEBUG) Log.d(TAG, "supportMinOrMaxDimesion");
        try
        {
    ...
    {
        if(DEBUG) Log.d(TAG, methodName + " == " + percentVal);
        if (percentVal != null)
        {
                                                         ...
                                                         AttributeSet attrs)
    {
        ...
        percentVal = getPercentVal(array,
                ...
                false);
        if(DEBUG) Log.d(TAG,"minHeight = "+percentVal);
        if (percentVal != null)
        {
            checkForInfoExists(info);
        ...
        if (Log.isLoggable(TAG, Log.DEBUG))
        {
            if(DEBUG) Log.d(TAG, "constructed: " + info);
        }
        return info;
    }
    ...
    public void restoreOriginalParams()
    {
        for (int i = 0, N = mHost.getChildCount(); i < N; i++)
        {
            ...
            if (Log.isLoggable(TAG, Log.DEBUG))
            {
                if(DEBUG) Log.d(TAG, "should restore " + view + " " + params);
            }
            if (params instanceof PercentLayoutParams)
            {
                PercentLayoutInfo info =
    ...
    public boolean handleMeasuredStateTooSmall()
    {
        ...
        for (int i = 0, N = mHost.getChildCount(); i < N; i++)
        {
            ...
            if (Log.isLoggable(TAG, Log.DEBUG))
            {
                if(DEBUG) Log.d(TAG, "should handle measured state too small " + view + " " + params);
            }
            if (params instanceof PercentLayoutParams)
            {
                PercentLayoutInfo info =
        ...
        if (Log.isLoggable(TAG, Log.DEBUG))
        {
            if(DEBUG) Log.d(TAG, "should trigger second measure pass: " + needsSecondMeasure);
        }
    ...
    private static boolean shouldHandleMeasuredWidthTooSmall(View view, PercentLayoutInfo info)
    {
        ...
        return state == ViewCompat.MEASURED_STATE_TOO_SMALL && (info.widthPercent!=null&&info.widthPercent.percent >= 0) &&
    ...
    private static boolean shouldHandleMeasuredHeightTooSmall(View view, PercentLayoutInfo info)
    {
        ...
        return state == ViewCompat.MEASURED_STATE_TOO_SMALL && (info.heightPercent!=null&&info.heightPercent.percent >= 0) &&
    ...
    public static class PercentLayoutInfo
    {
                                           ...
                                           int heightHint)
        {
            ...
            if (Log.isLoggable(TAG, Log.DEBUG))
            {
                if(DEBUG) Log.d(TAG, "after fillMarginLayoutParams: (" + params.width + ", " + params.height
                        + ")");
            }
        }
